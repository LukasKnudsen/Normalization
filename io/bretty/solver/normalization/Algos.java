...
public class Algos {
	...
	public static List<Relation> threeNFDecompose(Relation relation) {
		Set<FuncDep> basis = minimalBasis(relation.getFds());
		List<Relation> result = new ArrayList<>();

		for (FuncDep fd : basis) {
			Set<Attribute> attrs = new HashSet<>(fd.getLeft());
			attrs.addAll(fd.getRight());
			Set<FuncDep> fds = projection(attrs, basis);
			result.add(new Relation(attrs, fds));
		}

		// Add a candidate key if none of the above relations contains it
		Set<Set<Attribute>> keys = keys(relation.getAttrs(), relation.getFds());
		boolean keyCovered = false;
		for (Set<Attribute> key : keys) {
			for (Relation r : result) {
				if (r.getAttrs().containsAll(key)) {
					keyCovered = true;
					break;
				}
			}
			if (!keyCovered) {
				Set<FuncDep> keyFds = projection(key, basis);
				result.add(new Relation(key, keyFds));
			}
		}
	...
	public static List<Relation> BCNFDecompose(Relation relation) {
		List<Relation> result = new ArrayList<>();
		result.add(relation);
		...
		do {
			changed = false;
			List<Relation> next = new ArrayList<>();

			for (Relation r : result) {
				Set<FuncDep> violations = checkBCNF(r.getAttrs(), r.getFds());
				if (violations.isEmpty()) {
					next.add(r);
				} else {
					changed = true;
					FuncDep violating = violations.iterator().next();

					Set<Attribute> closure = closure(violating.getLeft(), r.getFds());

					Set<Attribute> r1Attrs = new HashSet<>(violating.getLeft());
					r1Attrs.addAll(violating.getRight());

					Set<Attribute> r2Attrs = new HashSet<>(r.getAttrs());
					r2Attrs.removeAll(violating.getRight());
					r2Attrs.addAll(violating.getLeft());

					next.add(new Relation(r1Attrs, projection(r1Attrs, r.getFds())));
					next.add(new Relation(r2Attrs, projection(r2Attrs, r.getFds())));
					break; // Decompose one at a time
				}
			}
